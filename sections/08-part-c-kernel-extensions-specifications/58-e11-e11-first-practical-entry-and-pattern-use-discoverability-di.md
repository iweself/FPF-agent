...
| Human and AI-assisted retrieval vs authority | Retrieval may return helpful fragments, but fragments must not answer as if they were the strongest applicable pattern body. |
...
- avoid common wrong patterns, wrong strongest applicable pattern bodies, or projection-only
...
reclassification`, `admissible entry stop`, `thin echo`, and `strongest applicable pattern body`.
...
| --- | --- |
| first bringing candidate patterns into view through reader words, domain phrases, or query cues | lexical-query support under `F.17 / F.18 / E.10`, coordinated by `E.11` only where the pattern-entry load is live |
| one description's first-contact recognition, truthful applicability signal, or definition authority source | `A.6.RSIG` |
| choosing among patterns, candidate patterns, nearby patterns, wrong strongest applicable pattern bodies, or entry-load reclassifications inside the pattern language | `E.11` |
...
| one compact or worked projection of already-governed pattern-entry discoverability content | `J.4`, `I.2`, `Preface`, the pattern `Problem frame`, or lexical support according to the strongest-governing-role map |
...
| one actual work sequence, method, plan, episteme publication, work product, or execution stop | the relevant method/work pattern rather than `E.11` |
...
the relevant lexical/naming pattern or authority source.
...
Each entry/discoverability claim names one strongest applicable pattern body or strongest
projection role. Other mentions remain thin echoes.

| Claim payload | Strongest governing pattern/source role or projection role | Thin echoes allowed in |
| --- | --- | --- |
...
Support-role parity means semantic consistency of first-contact entry load, strongest
governing pattern/source role or strongest projection role, wrong-pattern boundary, projection-only
...
strongest applicable pattern body / projection role:
...
2. it names or implies the first candidate pattern or authority source;
...
3. tempting wrong pattern = minting one second `U.Episteme` for the same claim or one parallel rule
   lane;
4. disambiguating fact = the governed `U.Episteme` or authored unit stays the same; only rendering,
...
- **CC-E11-3 Strongest home / thin echo.** Each entry/discoverability claim has
  one strongest applicable pattern body or strongest projection role; other mentions remain thin
  echoes.
