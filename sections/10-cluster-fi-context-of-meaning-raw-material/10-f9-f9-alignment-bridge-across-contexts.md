...
**Use this when.** Use this pattern when two local senses from different contexts need one explicit bridge card before a team can admissibly reuse a label, justify a row, or compare the cases without pretending they are simply the same thing.
...
- a CSC/RT/C.26 coarsening handoff when the exported representation is intentionally weaker.
...
**Local stance.** Best-known current practice supports a narrow rule: cross-context reuse is admissible only when correspondence is typed, directional where needed, explicit about loss, and weaker than silent lexical identity or convenience equivalence.
