...
- `Tempting wrong pattern, wrong definition authority source, or false family`
- `Disambiguating fact`
- `Recognition repair or entry-load reclassification`
- `Actual strongest applicable pattern body or projection role`
...
- **Initial uncertainty:** the reader may think FPF starts with the whole
  specification, but the entry load is narrower: align context, roles, method/work
  vocabulary, and actual execution.
- **Plausible candidate patterns:** `A.1.1`, `A.15`, `A.15.2`, `A.15.3`,
  `B.5.1`.
- **Nearby patterns:** `F.11` for method/work vocabulary; `F.9` for bridge
...
- **Actual strongest applicable pattern body or projection role:** `A.15` and its neighbors govern
  the role/method/plan/run split; `F.17` is a stabilizing lexical projection,
  not the whole alignment home.
- **Admissible entry stop:** the reader has opened the right alignment home or has
  enough first shared vocabulary to proceed.
- **What not to infer:** no universal first chain is implied.

### I.2.2 - Partly-said / language-state discovery

- **Case signal:** "This phrase matters, but it is not yet a claim."
- **Initial uncertainty:** the reader can be seeing one cue, one early
  language-state burden, one publication seam, or one routed claim family.
...
- **Tempting wrong pattern:** harden the cue into `A.6.P`, `A.6.Q`, `A.6.A`,
  ...
  object.
- **Disambiguating fact:** the phrase still needs preservation and burden
...
- **Decision movement:** if the phrase is still a cue, stay in `C.2.LS` /
  ...
  forced into a quality/action endpoint too early, reject `A.6.Q`, `A.6.A`, and
  `C.25` for now.

- **Actual strongest applicable pattern body or projection role:** `C.2.LS` / `A.16*` /
  ...
  actually live.
- **Admissible entry stop:** cue preserved, entry plurality opened, or burden
...
### I.2.3 - Boundary unpacking / claim routing

- **Case signal:** "The API/contract says X."
...
- **Nearby patterns:** `A.6.RSIG` if first-contact recognition is still live;
  ...
  action invitation wording; `E.17` for publication/view burden.
- **Tempting wrong pattern:** treat an API/access phrase as a promise of
  downstream effect, or treat one boundary phrase as the whole contract.
- **Disambiguating fact:** the sentence mixes law/gate/duty/evidence/action
  burdens, or the encountered description's definition authority source is not yet
  clear.
...
- **Actual strongest applicable pattern body or projection role:** `A.6.B` / `A.6.C` govern
  routed claim decomposition; `A.6.RSIG` only governs first-contact
  description recognition.
- **Admissible entry stop:** boundary claim pattern opened, or one Claim Register /
  routed atomic claim set is ready for the next authoritative pattern.
- **What not to infer:** one contract/API cue does not by itself create one
  action, quality, or evidence home.

### I.2.4 - Admissible comparison / pool / selection / selected-set publication
...
- **Nearby patterns:** `C.11` if the entry load narrows to one local decision
  ...
  return; `A.19.CPM` / `A.19.SelectorMechanism` if comparator/selector
  structure is live.
...
- **Decision movement:** if the work is still forming a candidate pool, inspect
  ...
  `G.5`; if the burden has narrowed to choosing one option under a local
  decision doctrine, inspect `C.11`; if the missing object is a call plan or
  planned comparison setup, inspect `C.24`.

- **Actual strongest applicable pattern body or projection role:** `C.19` for candidate-pool
...
### I.2.5 - Generator / SoTA / portfolio kit
...
- **Nearby patterns:** `B.5.2.1` and `C.17-C.19` when creative search,
  novelty, or explore/exploit policy is already central; `G.10` / `G.11` when
...
- **Disambiguating fact:** the first deliverable must be reusable across
  searches, portfolios, or updates.
- **Actual strongest applicable pattern body or projection role:** `G.1` / `G.2` for generator and
...
### I.2.6 - Same-entity rewrite / explanation / comparative reading

- **Case signal:** "We need to explain the same thing for another audience."
...
- **Nearby patterns:** `E.17.AUD.LHR` for pressured-head local repair;
  `E.17.AUD.OOTD` for authored-unit stability.
...
- **Actual strongest applicable pattern body or projection role:** `A.6.3.CR` for same-entity
...
- **Initial uncertainty:** the reader may be seeing ordinary prose, one state/snapshot, one measured rate, one intervention-sensitive temporal claim, or a stronger benchmark, quality, viability, promise, causal/evaluation, dynamics-law, or residual QL question.
...
- **Disambiguating fact:** the phrase changes supported use only when effort, window, resistance/cost, basis, and reopen condition matter for action.
- **Recognition repair or question reclassification:** keep a snapshot as Dyn0; keep a measured trend/rate as Dyn1 and inspect `C.16` when measurement construction or comparability is live; use `Dyn2TemporalClaimAdequacyCard` when the intervention-sensitive temporal claim itself changes supported use; move the stronger question to the named FPF pattern that carries it.
- **Actual strongest pattern or projection role:** `C.27` carries authored temporal-claim adequacy; `C.16` carries measurement construction and comparability; `A.3.3` carries reusable transition law or formal dynamics model; `G.9` carries benchmark parity; `C.25` carries quality-family adequacy; `C.26.3` carries viability-envelope regulation; `C.26` carries residual QL reading only after ordinary temporal, measurement, work, benchmark, proxy, and dynamics readings are exhausted.
- **Admissible entry stop:** ordinary prose, Dyn0, Dyn1/C.16, a local `Dyn2TemporalClaimAdequacyCard`, a boundary-crossing `Dyn2TemporalClaimProfile`, or a named stronger FPF pattern relation.
...
### I.2.8 - Causal-use / counterfactual-support repair
...
- **Initial uncertainty:** the reader may be seeing association, a metric disparity, temporal change, method/work execution, simulation output, deontic boundary language, or a real causal-use claim.
