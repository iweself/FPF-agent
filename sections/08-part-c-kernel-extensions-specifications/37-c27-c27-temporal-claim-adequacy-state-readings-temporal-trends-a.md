...
- what use the claim can carry and what downstream claim/effect/use remains unsupported;
...
- the live issue is authored-unit drift: one overloaded local head, drifting
  primary described entity, bounded comparison, explanation faithfulness, or
  approval/action wording should use E.17.AUD, E.17.ID.CR, E.17.EFP, or the
  pattern that governs the downstream claim/effect/use before C.27;
...
need a dynamics pattern relation if a downstream claim/effect/use is claimed.
...
unsupportedUse / reopen:
  What downstream claim/effect/use is unsupported, and what would reopen, downgrade, or add a pattern reference to this claim?
```
...
dyn2CoastingClaimBlock? / Dyn2TemporalClaimAdequacyCard fields:
  ...
  unsupportedUse : what downstream claim/effect/use this coasting reading does not support
...
- `dyn2PromiseBoundaryRoute?` states only the temporal move, window,
  supported use, unsupported downstream claim/effect/use, and references to the patterns that
...
- `dyn2HighStakesTemporalMoveRoute?` states only the high-stakes temporal move, window,
  unsupported downstream claim/effect/use, and reference to the pattern that carries the harm,
...
residual probe/frame/order/export/coarsening cue still changes the admissible
...
| evidence or assumption | The basis that makes the `Dyn2TemporalClaimAdequacyCard` more than a slogan: observed trace, measurement, work evidence, model assumption, planning assumption, or diagnostic judgement. | Cites C.16, work/evidence, causal, benchmark, or assurance patterns when a downstream claim/effect/use is claimed. |
...
| causalAssumptionSetRef | Assumptions under which the causal/model/evaluation claim holds. | Routes to `C.28`; not hidden inside C.27 shorthand. |
| rivalCauseSetRef | Alternative causes that could explain observed rate-change. | Routes to `C.28`; required when causal reading is live. |
| causalIdentificationProfileRef | Identification strategy, graph/calculus route, data-regime route, or bound used for the causal claim. | Routes to `C.28`; absent identification limits supported causal use. |
| causalUseEvidenceDesignRef | Experiment, quasi-experiment, target-trial emulation, counterfactual sampling, simulation validation, or other causal evidence-design reference. | Routes to `C.28`; absent design limits supported causal use. |
| offPolicyCausalEvaluationProfileRef | Off-policy/sequential/adaptive policy evaluation route when rate-change or policy-improvement wording depends on logged behavior or replay. | Routes to `C.28`; C.27 does not own off-policy causal evaluation. |
| supportedCausalUse | The causal conclusion or decision use carried by the `C.28` causal-use relation. | Must be no stronger than design, assumptions, outcome evidence, and uncertainty. |
...
| C.27 resistance/inertia block | `dyn2ResistanceInertiaBlock?` names resistance proxy family, resistance proxy basis posture, evidence, and unsupported downstream claim/effect/use; `resistanceProxyBasisPosture = unknown` may carry local diagnostic use but blocks durable acceleration, causal, benchmark, promise-like, or assurance use. |
...
| Benchmark / publication | "Method A improves faster than Method B and should be published as superior." | `Dyn2TemporalClaimProfile` or pattern reference is justified: G.9 benchmark parity, C.16 measurement, possible `C.28` causal-use relation, and C.27 dynamic-claim relation declaration. |
...
| Harmful acceleration / viability | "Faster rollout improved release velocity while support load and recovery time degraded." | C.27 names acceleration/braking/throttling/recovery timing and unsupported downstream claim/effect/use; C.26.3, C.25, assurance, safety, legal, ethics, or wellbeing patterns carry the envelope or harm claim. |
...
unsupported downstream claim/effect/use.
...
| Card minimum | A `Dyn2TemporalClaimAdequacyCard` names target, move, intervention, window, resistance/cost, basis, supported use, unsupported downstream claim/effect/use, and reopen/pattern-reference condition. |
...
| Governing-pattern relation | C.27 does not carry measurement, transition law, Work actuals, planning, `C.28`-governed causal-use claim, benchmark parity, promise/boundary, assurance, or QL residue. |
...
| Exit | If no downstream claim/effect/use changes, the claim exits as ordinary prose, Dyn0/Dyn1 reading, C.16 measurement, `U.Dynamics`, or another governing pattern. |
...
| C.27 as `C.28`-governed causal-use claim | Rate changed after effort, therefore effort caused it. | Mark planning/diagnostic posture or include `dyn2CausalUseRoute?` with `causalInterventionSpecRef`, contrast/counterfactual, timing, outcome, assumptions, rival causes, supported/unsupported causal use, and `C.28` causal-use relation. |
...
The source basis is strongest where it replaces
...
downstream claim/effect/use fields only when the claim depends on coordination between bearers.
...
| Rhythm is decorative cadence/vibe. | Rhythm work needs bearer, anchor, window, basis/proxy, and supported use; coupling belongs only in downstream claim/effect/use fields. | Use `Dyn2TemporalClaimAdequacyCard`; include coupling/phase/entrainment only when the claim depends on cross-bearer relation. |
...
unsupported downstream claim/effect/use, and choose the weaker C.27 output or the cited stronger
pattern relation.
...
> for what supported use, and what downstream claim/effect/use is unsupported. Only boundary-crossing
